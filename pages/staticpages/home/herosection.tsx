...
const EditHeroSection = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Edit Hero Section'));
    });

    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl' ? true : false;
    const { t } = useTranslation();
    const [formData, setFormData] = useState({
        pageName: 'Look & Feel Your Best Anytime, Anywhere',
        pageContent: '',
    });
    ...
    const { mutate: updateHeroSectionSec, isLoading: isUpdatingHeroSectionSec } = useUpdateSectionAction();
            ...
            if (id) {
                updateHeroSectionSec(
                    ...
                    },
                    {
                        onSuccess: () => {
                            router.push('/staticpages/home');
                            ErrorToast(t('Hero Section updated successfully'), 'success', t('Success')?.toString());
                        },
                        onError: (error) => {
                            ErrorToast(t('Failed to update Hero section. Please try again.'), 'error', t('Error')?.toString());
                        },
                    }
                );
            }
            ...
            <div className="pt-5">
                <div className="mb-5 grid grid-cols-1 gap-5 lg:grid-cols-3">
                    <div className="flex flex-col gap-6 lg:col-span-3">
                        <div className="panel">
                            <div className="mb-5">
                                <h5 className="font-['Hermann'] rtl:font-['Tajawal'] rtl:font-medium text-2xl font-bold uppercase text-[#012d22] dark:text-white-light">{t("Edit Hero Section")}</h5>
                            </div>
                                ...
                                <form className="space-y-5" onSubmit={formik.handleSubmit}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        {filteredPermissions?.[0].meta.pivot_update === 1 && (
                                            <Button
                                                text={isUpdatingHeroSectionSec ? t("Saving") : t("Save")}
                                                type='submit'
                                                bgColor="#012d22"
                                                textColor="#ffff"
                                            />
