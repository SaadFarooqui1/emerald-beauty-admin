...
const EditContactFormContent = () => {

    const { t } = useTranslation();

    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(setPageTitle('Edit Contact Form Section'));
    });
    ...
    const [formData, setFormData] = useState({
        ...
        description: "We'd love to hear from you! If you have any questions or need assistance, feel free to reach out.",
    });
    ...
    const handleCancelClick = () => {
        router.push("/staticpages/contactus");
    };
    ...
    const { contactUsSchema } = useValidationSchemaHook();
    const { mutate: updateFormSection, isLoading: isUpdatingFormSection } = useUpdateSectionAction();
            ...
            if (id) {
                updateFormSection(
                    ...
                    },
                    {
                        onSuccess: () => {
                            router.push('/staticpages/contactus');
                            ErrorToast(t('Form section updated successfully'), 'success', t('Success')?.toString());
                        },
                        onError: (error) => {
                            ErrorToast(t('Failed to update form section. Please try again.'), 'error', t('Error')?.toString());
                        },
                    }
                );
            }
    ...
    return (
        <div>
            <ul className="flex space-x-2 rtl:space-x-reverse">
                <li>
                    <Link href="/staticpages/contactus" className="font-['Raleway'] rtl:font-['Tajawal'] rtl:font-medium text-lg font-semibold text-[#1bd9bf] hover:underline dark:text-white-light">
                        {t("Contact Form")}
                    </Link>
                </li>
            ...
            <div className="pt-5">
                <div className="mb-5 grid grid-cols-1 gap-5 lg:grid-cols-3">
                    <div className="flex flex-col gap-6 lg:col-span-3">
                        <div className="panel">
                            <div className="mb-5">
                                <h5 className="font-['Hermann'] rtl:font-['Tajawal'] rtl:font-medium text-2xl font-bold uppercase text-[#012d22] dark:text-white-light">{t("Edit Contact Form Section")}</h5>
                            </div>
                            ...
                            <div className="mb-5">
                                <form className="space-y-5" onSubmit={formik.handleSubmit}>
                                    <div className="grid grid-cols-1 gap-6">
                                        ...
                                        <>
                                            ...
                                            <div>
                                                <InputField
                                                    ...
                                                    type="textarea"
                                                    placeholder={t('Enter Description')?.toString()}
                                                    defaultValue={formik.values.arabic_description}
                                                    rows={5}
                                                    onChange={formik.handleChange}
                                                    onBlur={formik.handleBlur}
                                                />
                                        ...
                                        <FileUpload
                                            label={t("Cover Picture")?.toString()}
                                            name="path"
                                            accept="image/*"
                                            maxFileSize="10 MB"
                                    ...
                                    <div className="flex items-center gap-2">
                                        {filteredPermissions?.[0].meta.pivot_update === 1 && (
                                            <Button
                                                text={isUpdatingFormSection ? t("Saving") : t("Save")}
                                                type='submit'
                                                bgColor="#012d22"
                                                textColor="#ffff"
                                            />
...
export default EditContactFormContent;
