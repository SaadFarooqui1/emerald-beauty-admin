...
const EditOurStoryContent = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Edit Banner'));
    });
    ...
    const { bannerSchema } = useValidationSchemaHook();
    const { mutate: updateBanner, isLoading: isUpdatingBanner } = useUpdateSectionAction();
            ...
            if (id) {
                updateBanner(
                    ...
                    },
                    {
                        onSuccess: () => {
                            router.push('/staticpages/services');
                            ErrorToast(t('Banner updated successfully'), 'success', t('Success')?.toString());
                        },
                        onError: (error) => {
                            ErrorToast(t('Failed to update Banner. Please try again.'), 'error', t('Error')?.toString());
                        },
                    }
                );
            }
            ...
            <div className="pt-5">
                <div className="mb-5 grid grid-cols-1 gap-5 lg:grid-cols-3">
                    <div className="flex flex-col gap-6 lg:col-span-3">
                        <div className="panel">
                            <div className="mb-5">
                                <h5 className="font-['Hermann'] rtl:font-medium rtl:font-['Tajawal'] text-2xl font-bold uppercase text-[#012d22] dark:text-white-light">{t("Edit Banner")}</h5>
                            </div>
                            ...
                            <div className="mb-5">
                                <form className="space-y-5" onSubmit={formik.handleSubmit}>
                                    <div className="grid grid-cols-1 gap-6">
                                        ...
                                        /> */}
                                        <FileUpload
                                            label={t("Banner Picture")?.toString()}
                                            name="path"
                                            accept="image/*"
                                            maxFileSize="10 MB"
                                    ...
                                    <div className="flex items-center gap-2">
                                        {filteredPermissions?.[0].meta.pivot_update === 1 && (
                                            <Button
                                                text={isUpdatingBanner ? t("Saving") : t("Save")}
                                                type='submit'
                                                bgColor="#012d22"
                                                textColor="#ffff"
                                            />
