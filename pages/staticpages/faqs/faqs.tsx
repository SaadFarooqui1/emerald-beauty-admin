...
import { useDeleteFaqAction, useModelGetFaqs } from '@/models/faqs.model';
...
const rowData = [
    ...
    },
    {
        ...
        answer: 'You can cancel your order from your account dashboard or contact customer support.',
    },
    {
        id: 5,
...
const FAQs = () => {
    ...
    const { data, isLoading, isError, refetch } = useModelGetFaqs({
        page,
        "per-page": pageSize,
        search,
    });
    ...
    const { mutate: deleteFaq, isLoading: isDeleting } = useDeleteFaqAction();
            ...
            deleteFaq(
                { id: deleteRecordId },
                {
                    onSuccess: () => {
                        ...
                        ErrorToast(t('Faq deleted successfully'), 'success', t('Success')?.toString());
                        refetch(); // Refresh the list
                    },
                    onError: (error) => {
                        // console.error("Error deleting Faq:", error);
                        setDeleteModal(false);
                        ErrorToast(t('Failed to delete Faq. Please try again.'), 'error', t('Error')?.toString());
                    },
                }
            );
        }
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-between gap-2 md:gap-0">
                <div>
                    <h4 className="text-[#012d22] text-2xl font-bold font-['Hermann'] rtl:font-['Tajawal'] rtl:font-medium uppercase dark:text-white-light">{t("FAQs")}</h4>
                </div>
            </div>
            <div className="panel mt-6">
                <div className="mb-5 flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                    <h5 className="text-xl font-bold text-[#012d22] font-['Hermann'] rtl:font-['Tajawal'] rtl:font-medium uppercase dark:text-white-light">{t("FAQs List")}</h5>
                    {filteredPermissions?.[0].meta.pivot_create === 1 && (
                        <div>
                            <button type="button" className="btn shadow-none bg-[#012d22] text-white rounded-full font-semibold font-['Raleway] rtl:font-['Tajawal'] rtl:font-medium text-lg px-3" onClick={() => router.push("/staticpages/faqs/addfaq")}>
                                {t("Add FAQ")}
                            </button>
                        </div>
                    )}
                ...
                <div className="datatables">
                    <DataTable
                        ...
                        columns={[
                            ...
                            { accessor: 'question', title: t('Question') },
                            { accessor: 'answer', title: t('Answer') },
                            {
                                accessor: 'actions',
                                title: t('Actions'),
                                render: (record) => (
                                    <div className="flex items-center gap-3">
                                        {filteredPermissions?.[0].meta.pivot_update === 1 && (
                                            <button
                                                ...
                                                onClick={() => { router.push(`/staticpages/faqs/editfaq?id=${record.id}`) }}
                                            >
