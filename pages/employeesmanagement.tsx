...
const Custom = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Employees Management'));
    });
            ...
            deleteUser(
                { id: deleteRecordId },
                {
                    onSuccess: () => {
                        ...
                        ErrorToast(t('Employee deleted successfully'), 'success', t('Success')?.toString());
                        refetch(); // Refresh the list
                    },
                    onError: (error) => {
                        ErrorToast(t('Failed to deleting Employee. Please try again.'), 'error', t('Error')?.toString());
                        setDeleteModal(false);
                    },
                }
            );
    ...
    const filteredPermissions = user?.roles?.[0]?.permissions?.filter(
        (permission: any) => permission.meta.pivot_module_id === 70
    );
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-between gap-2 md:gap-0">
                <div>
                    <h4 className="text-[#012d22] text-2xl font-bold font-['Hermann'] rtl:font-['Tajawal'] rtl:font-medium uppercase dark:text-white-light">{t("Employees Management")}</h4>
                </div>
            ...
            <div className="panel mt-6">
                <div className="mb-5 flex flex-col gap-3 sm:flex-row items-center justify-center sm:justify-between">
                    <h5 className="text-xl font-bold text-[#012d22] font-['Hermann'] rtl:font-['Tajawal'] rtl:font-medium uppercase dark:text-white-light">{t("Employee List")}</h5>
                    {filteredPermissions?.[0].meta.pivot_create === 1 && (
                        <div>
                            <button type="button" className="btn shadow-none bg-[#012d22] text-white rounded-full font-semibold font-['Raleway] rtl:font-['Tajawal'] rtl:font-medium text-lg px-3" onClick={() => router.push('/employeesmanagement/employee/add')}>
                                {t("Add New Employee")}
                            </button>
                        </div>
                    )}
                    ...
                    <DataTable
                        ...
                        columns={[
                            ...
                            },
                            {
                                accessor: 'shifthours',
                                title: t('Shift Hours'),
                                render: (record) => `${record?.shift_start_time}-${record?.shift_end_time}`,
                            },
                                    ...
                                    <div className="flex items-center gap-3">
                                        {filteredPermissions?.[0].meta.pivot_update === 1 && (
                                            <button type="button" className="btn shadow-none bg-[#012d22] text-white rounded-full font-normal font-['Raleway] rtl:font-['Tajawal'] px-3" onClick={() => {
                                                router.push({
                                                    pathname: `/employeesmanagement/employee/edit`,
                                                    query: { id: record?.id },
                                                });
                                            }}>
