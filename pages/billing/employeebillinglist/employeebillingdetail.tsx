...
import { useModelGetSalaries } from '@/models/salary.model';
...
const EmployeeBillingDetail = () => {
    const dispatch = useDispatch();
    const router = useRouter();
    useEffect(() => {
        dispatch(setPageTitle('Employee Billing Detail'));
    });
    ...
    const { data, isLoading, isError, refetch } = useModelGetSalaries({
        user_id: id,
    });
        ...
        <div>
            <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-between gap-2 md:gap-0">
                <ul className="flex space-x-2 rtl:space-x-reverse">
                    <li>
                        <Link href="/billing/employeebillinglist" className="font-['Raleway'] rtl:font-['Tajawal'] rtl:font-medium text-lg font-semibold text-[#1bd9bf] hover:underline dark:text-white-light">
                            {t("Employee List")}
                        </Link>
                    </li>
                ...
                <div className="datatables">
                    <DataTable
                        ...
                        columns={[
                            ...
                            {
                                accessor: 'month', title: t('Month'),
                                render: (record) => {
                                    // Define an array of month names (assuming 1 = January, 2 = February, etc.)
                                    const monthNames = [
                                        'January', 'February', 'March', 'April', 'May', 'June',
                                        'July', 'August', 'September', 'October', 'November', 'December'
                                    ];
                                    // Convert the API's month (assumed to be a number or string) to a 0-based index
                            ...
                            { accessor: 'salary', title: t('Salary') },
                            {
                                ...
                                render: (record) => (
                                    <div className="flex items-center gap-3">
                                        <button type="button" className="btn shadow-none bg-[#012d22] text-white rounded-full font-normal font-['Raleway] rtl:font-['Tajawal'] px-3" 
                                            onClick={() => router.push({
                                                pathname: '/billing/employeebillinglist/employeebillingdetail/paymentinfo',
                                        ...
                                        {record.payment_status_text === 'Pending' && (
                                            <button type="button" className="btn shadow-none bg-[#1bd9bf] text-[#012d22] rounded-full font-normal font-['Raleway] rtl:font-['Tajawal'] px-3" onClick={() => {
                                                router.push({
                                                    pathname: `/billing/employeebillinglist/employeebillingdetail/paynow`,
                                                    query: { id: record.id, name: router?.query?.name, salary: record.salary },
                                                });
                                            }}>
                                                {t("Pay Now")}
                                            </button>
...
EmployeeBillingDetail.authenticate = true;
export default EmployeeBillingDetail;
