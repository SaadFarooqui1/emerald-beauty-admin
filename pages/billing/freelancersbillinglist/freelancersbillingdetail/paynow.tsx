...
import { useUpdateBookingPaymentStatusAction } from '@/models/booking.model';
import { getCookie } from 'cookies-next';
import { ErrorToast } from '@/utils/helper';
import { useTranslation } from 'react-i18next';

const FreelancersPayNow = () => {
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(setPageTitle('Freelancers PayNow'));
    });
    ...
    const { name, comission } = router.query;


    const handleCancelClick = () => {
        router.push("/billing/freelancersbillinglist/freelancersbillingdetail");
    };
    ...
    const { mutate: updateBooking, isLoading: isCreatingBooking } = useUpdateBookingPaymentStatusAction();
        ...
        onSubmit: (values) => {
            if (id) {
                updateBooking(
                    ...
                    {
                        onSuccess: () => {
                            router.push('/billing/freelancersbillinglist');
    ...
    return (
        <div>
            <ul className="flex space-x-2 rtl:space-x-reverse">
                <li>
                    <button className="font-['Raleway'] text-lg font-semibold text-[#1bd9bf] hover:underline dark:text-white-light"
                        onClick={()=> router.back()}
                    >
                        {name}
                    </button>
                                ...
                                <form className="space-y-5" onSubmit={formik.handleSubmit}>
                                    ...
                                    <div className="grid grid-cols-1 gap-7">
                                        <div>
                                            ...
                                            />
                                            {formik.errors.reciept && formik.touched.reciept && <div className='text-red-500 rtl:font-["Tajawal"] font-normal'>{formik.errors.reciept}</div>}
                                        </div>
                                    </div>
...
FreelancersPayNow.authenticate = true;
export default FreelancersPayNow;