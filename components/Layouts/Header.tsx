...
import { toggleLocale, toggleRTL, toggleSidebar, toggleTheme } from '../../store/themeConfigSlice';
import Dropdown from '../Dropdown';
import LogOutComponent from '../log-out';
import Image from 'next/image';
import logob from 'public/assets/images/logo_b.svg'
import logotext from 'public/assets/images/logotext.svg'
import logotextdark from 'public/assets/images/logotextdark.svg'
import { useModelGetAllNotifications, useModelGetUnreadCount, useUpdateMarkAllReadAction, useUpdateMarkReadAction } from '@/models/notification.model';
...
const Header = () => {
    ...
    // const { data: notifications_paginate, isLoading: notificationsLoading, isError: notificationsError, refetch: notificationsRefetch } = useModelGetAllNotifications({});

    // const notifications = notifications_paginate?.data || [];
    // const totalRecords = notifications_paginate?.meta?.total || 0;

    // // Update notificationsData when the data is fetched or refetched
    // useEffect(() => {
    ...
    //     setNotificationsData(prevNotifications => prevNotifications.filter((notification) => notification.id !== id));
    // };


    const { data, refetch } = useModelGetUnreadCount({});

    const unreadCount = data?.unread_count || 0;


    const { mutate: updateMardRead, isLoading: isUpdatingMarkRead } = useUpdateMarkReadAction();

    // const handleToggle = () => {
    //     // Set the record ID first
    //     updateMardRead(record.id);
    ...
    //                 ErrorToast(`User ${record.is_active ? 'Deactivate' : 'Activate'} successfully!`, 'success', 'Success');
    //                 refetch(); // Refresh the list
    //             },
    //             onError: (error) => {
    //                 ErrorToast(`Failed to ${record.is_active ? 'Deactivate' : 'Activate'} user. Please try again.`, 'error', 'Error');
    //             },
    //         }
    //     );
    // };

    const { mutate: updateMarkAllRead, isLoading: isUpdatingMarkAllRead } = useUpdateMarkAllReadAction();
    ...
    return (
        <header className={`z-40 ${themeConfig.semidark && themeConfig.menu === 'horizontal' ? 'dark' : ''}`}>
            <div className="shadow-sm">
                <div className="relative flex w-full items-center bg-white px-4 sm:px-5 py-2.5 dark:bg-black">
                    <div className="horizontal-logo flex items-center justify-between ltr:mr-1 rtl:ml-1 lg:hidden">
                        <Link href="/" className="main-logo flex shrink-0 items-center gap-2">
                            <Image className="inline ltr:-ml-1 rtl:-mr-1 w-8 sm:w-10" src={logob} alt="logo" />
                            <div className='hidden sm:block'>
                                <Image className="inline dark:hidden w-32" src={logotext} alt="logo" />
                                <Image className="dark:inline hidden w-32" src={logotextdark} alt="logo" />
                            </div>
                        </Link>
                        <button
                            type="button"
                            className="collapse-icon flex flex-none rounded-full bg-white-light/40 p-2 hover:bg-white-light/90 ltr:ml-2 rtl:mr-2 dark:bg-dark/40 dark:text-[#d0d2d6] dark:hover:bg-dark/60 lg:hidden"
                    ...
                    <div className="flex items-center space-x-1 sm:space-x-1.5 ltr:ml-auto rtl:mr-auto rtl:space-x-reverse dark:text-[#d0d2d6] sm:flex-1 ltr:sm:ml-0 sm:rtl:mr-0 lg:space-x-2">
                        <div className="sm:ltr:mr-auto sm:rtl:ml-auto">
                            ...
                            >
                                {/* <div className="relative">
                                    <input
                                        type="text"
                                        className="peer form-input focus:border-[#012d22] text-[#b8c3c0] font-normal font-['Raleway'] bg-gray-100 placeholder:tracking-widest ltr:pl-9 ltr:pr-9 rtl:pl-9 rtl:pr-9 sm:bg-transparent ltr:sm:pr-4 rtl:sm:pl-4 rounded-full"
                                        placeholder="Search here..."
                                    />
                                    <button type="button" className="absolute inset-0 h-9 w-9 appearance-none peer-focus:text-[#012d22] ltr:right-auto rtl:left-auto">
                                ...
                                <ul className="w-[300px] divide-y !py-0 text-dark dark:divide-white/10 dark:text-white-dark sm:w-[350px]">
                                    <li onClick={(e) => e.stopPropagation()}>
                                        <div className="flex items-center justify-between px-4 py-2 font-semibold">
                                            ...
                                            <span className="badge bg-primary/80">{unreadCount} Unread</span>
                                        </div>
                                    </li>
                                    {notifications.length > 0 ? (
                                        <>
                                            ...
                                            {unreadCount > 0 && (
                                                <li>
                                                    <div className="p-4">
                                                        <button className="btn btn-primary btn-small block w-full" onClick={handleMarkAllRead}>Read All Notifications</button>
                                                    </div>
                                                </li>
                                            )}
                                        </>
                        ...
                        <div className="dropdown flex shrink-0">
                            <Dropdown
                                ...
                                // button={<img className="h-9 w-9 rounded-full object-cover saturate-50 group-hover:saturate-100" src={user?.user_image} alt="userProfile" />}
                                button={<h5 className="h-9 w-9 rounded-full bg-[#1bd9bf] text-white text-lg content-center saturate-50 group-hover:saturate-100 capitalize" >{user?.first_name?.charAt(0)}</h5>}
                            >
                                <ul className="w-[230px] top-10 !py-0 font-semibold text-dark dark:text-white-dark dark:text-white-light/90">
                                    <li>
